        ...
        for template_idx, template_path in enumerate(template_paths):
            try:
                ...
                if effect_type is None:
                    current_effect = random.choice(["fog", "rain", "snow"])
                else:
    ...
    try:
        ...
        for img_path in input_images:
            ...
            for i, (patch, (row, col)) in enumerate(zip(patches, patch_info)):
                patch_path = os.path.join(temp_patches_dir, f"{os.path.basename(img_path).split('.')[0]}_patch_{row}_{col}.png")
        ...
        for idx, patch_path in enumerate(train_patches):
            ...
            for effect_type in ["fog", "rain", "snow"]:
